...
PRED_DATE = pd.Timestamp("2026-05-18")
BUY_DATE = pd.Timestamp("2026-05-19")
SELL_DATE = pd.Timestamp("2026-05-25")
...
def main():
    ...
    processed[feature_columns] = processed[feature_columns].replace([float("inf"), -float("inf")], pd.NA).fillna(0.0)
    ...
    for row in portfolio_df.itertuples(index=False):
        ...
        total += weighted_return
        rows.append({
            "stock_id": stock_id,
            "weight": weight,
            "buy_open_2026_05_19": buy_price,
            "sell_open_2026_05_25": sell_price,
            "return": ret,
            "weighted_return": weighted_return,
        })

    detail_df = pd.DataFrame(rows)
    out_path = ROOT / "output" / "check_0518_to_0525.csv"
